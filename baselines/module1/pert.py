...
class NewsFactorizationModule(nn.Module):
    """
    Module 1 — News Factorization với LERT-Large.

    Pipeline đơn giản:
        [CLS] embedding (per news)  +  stock factors
        → W_α ⊙ [X_n ; X_f]        (Eq. 4)
        → MLP per news              (Eq. 5)
    ...
    def __init__(
        ...
        self._news_feat_dim  = hidden_size          # [CLS] embedding
        ...
        # ── W_alpha (Eq. 4) ───────────────────────────────────────────────────
        self.W_alpha = nn.Parameter(torch.zeros(self._total_feat_dim))
...
def alpha_diversity_loss(model: NewsFactorizationModule, lambda_div: float = 0.05) -> torch.Tensor:
    ...
    w_news   = W[:model._news_feat_dim]
    w_factor = W[model._news_feat_dim:]
...
def train_epoch(model, loader, optimizer, device, lambda_div=0.05):
    ...
    for batch in pbar:
        ...
        loss = task_loss + 0.5 * kl_loss + div_loss
...
def load_checkpoint(path, model, optimizer=None, scheduler=None, device='cpu'):
    ...
    print(f"✅ Loaded checkpoint — epoch {epoch} | best_val_loss={best_val_loss:.4f}")
...
def main():
    ...
    CHECKPOINT_PATH = 'checkpoint_pert.pt'
    BEST_MODEL_PATH = 'best_pert.pt'
    RESUME = True

    MAX_LEN            = 192
    BATCH_SIZE         = 4       # T4 16GB: 2 | A100: 8
    EPOCHS             = 15
    LAMBDA_DIV         = 0.05
    WARMUP_EPOCHS      = 0
    PATIENCE           = 2
    ...
    optimizer = torch.optim.AdamW([
        {'params': enc_params,   'lr': 2e-6, 'weight_decay': 1e-2},
