...
# ══════════════════════════════════════════════════════════════════
# BƯỚC 1: CLEAN & NORMALIZE STOCK DATA
# ══════════════════════════════════════════════════════════════════
...
df['code_num'] = df['ts_code_x'].str.extract(r'^(\d+)').astype(str)
...
# ══════════════════════════════════════════════════════════════════
# BƯỚC 3: AGGREGATE NEWS SENTIMENT PER STOCK-DATE
# ══════════════════════════════════════════════════════════════════

df_news = df_news_all.copy()
df_news['code_num'] = df_news['CODE'].astype(str)

news_agg = (
    df_news
    .groupby(['code_num', 'trade_date'])[['pred_prob_neg', 'pred_prob_neu', 'pred_prob_pos']]
    .mean()
    .reset_index()
    .rename(columns={'trade_date': 'date'})
)
news_agg['news_score'] = news_agg['pred_prob_pos'] - news_agg['pred_prob_neg']

df = df.merge(news_agg[['code_num', 'date', 'news_score']], on=['code_num', 'date'], how='left')


# ══════════════════════════════════════════════════════════════════
# BƯỚC 4: SORT PORTFOLIOS — FF5 GROUPS + NEWS GROUP
# ══════════════════════════════════════════════════════════════════
...
tqdm.pandas(desc="Sorting FF5 portfolios")
...
sort_ff5 = eom[['ts_code_x', 'ym_next', 'size_grp', 'bm_grp', 'op_grp', 'inv_grp']].rename(
    columns={'ym_next': 'ym'}
)
df = df.merge(sort_ff5, on=['ts_code_x', 'ym'], how='left')

def assign_news_group_daily(grp):
    """
    Daily news group per paper Eq.6:
      P (Positive): top 20% news_score among stocks with news today
      N (Negative): bottom 20% news_score among stocks with news today
      M (Medium):   middle 60%, or stocks with no news
    """
    result = pd.Series('M', index=grp.index)
    has_news = grp['news_score'].notna()
    valid = grp[has_news]
    if len(valid) < 5:
        return result
    lo = valid['news_score'].quantile(0.20)
    hi = valid['news_score'].quantile(0.80)
    result[has_news & (grp['news_score'] <= lo)] = 'N'
    result[has_news & (grp['news_score'] >= hi)] = 'P'
    return result

df['news_grp'] = df.groupby('date', group_keys=False).apply(assign_news_group_daily)


# ══════════════════════════════════════════════════════════════════
# BƯỚC 5: VALUE-WEIGHTED PORTFOLIO RETURNS PER DAY
# ══════════════════════════════════════════════════════════════════
...
news_port = (
    df.dropna(subset=['size_grp', 'news_grp'])
    .groupby(['date', 'size_grp', 'news_grp'])
    .apply(vw_ret, include_groups=False)
    .reset_index()
)
news_port.columns = ['date', 'size_grp', 'news_grp', 'vw_ret']


# ══════════════════════════════════════════════════════════════════
# BƯỚC 6: COMPUTE FF5-NEWS FACTORS
# ══════════════════════════════════════════════════════════════════
...
news_piv = pivot_port(news_port, 'size_grp', 'news_grp')
...
SMB_news = (
    (safe_get(news_piv, ('S','P')) + safe_get(news_piv, ('S','M')) + safe_get(news_piv, ('S','N'))) / 3
    - (safe_get(news_piv, ('B','P')) + safe_get(news_piv, ('B','M')) + safe_get(news_piv, ('B','N'))) / 3
)

SMB = (SMB_bp + SMB_op + SMB_inv + SMB_news) / 4
...
# NEWS = M_t: (BP + SP)/2 - (BN + SN)/2  per paper
NEWS = (
    (safe_get(news_piv, ('B','P')) + safe_get(news_piv, ('S','P'))) / 2
    - (safe_get(news_piv, ('B','N')) + safe_get(news_piv, ('S','N'))) / 2
)
...
# ══════════════════════════════════════════════════════════════════
# BƯỚC 7: MONTHLY AGGREGATION
# ══════════════════════════════════════════════════════════════════

def compound_ret(s):
    return (1 + s).prod() - 1

factor_cols_all = ['MKT_RF', 'SMB', 'HML', 'RMW', 'CMA', 'NEWS',
                   'SMB_BP', 'SMB_OP', 'SMB_INV', 'SMB_NEWS']
...
# ══════════════════════════════════════════════════════════════════
# BƯỚC 8: MODULE2 FORMAT OUTPUT
# ══════════════════════════════════════════════════════════════════

stock_out = df[['ts_code_x', 'date', 'ret', 'ret_excess', 'total_mv']].copy()
stock_out['date_norm'] = stock_out['date'].dt.strftime('%Y-%m-%d')

stock_out = stock_out.merge(
    factors_daily[['MKT_RF', 'SMB', 'HML', 'RMW', 'CMA', 'NEWS']].reset_index(),
    on='date',
    how='left'
)
stock_out.rename(columns={'ret_excess': 'excess_return'}, inplace=True)

out_cols   = ['ts_code_x', 'date_norm', 'excess_return',
              'MKT_RF', 'SMB', 'HML', 'RMW', 'CMA', 'NEWS']
df_module2 = stock_out[out_cols].dropna(subset=['MKT_RF', 'SMB', 'HML', 'CMA', 'NEWS'])

print(factors_daily[['MKT_RF', 'SMB', 'HML', 'RMW', 'CMA', 'NEWS']].describe().round(6))
print(f"\nAnnualized factor means (×252):")
print((factors_daily[['MKT_RF', 'SMB', 'HML', 'RMW', 'CMA', 'NEWS']].mean() * 252).round(4))
...
factors_daily.to_csv(os.path.join(out_dir, 'ff5news_factors_daily.csv'))
factors_monthly.to_csv(os.path.join(out_dir, 'ff5news_factors_monthly.csv'))
df_module2.to_csv(os.path.join(out_dir, 'module2_data_ff5news.csv'), index=False)